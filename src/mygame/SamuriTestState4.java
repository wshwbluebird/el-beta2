 ...
 * @author wshwbluebird
 */
public class SamuriTestState4 extends AbstractAppState{
    private int clicktime = 0;
    private Vector3f locbase = new Vector3f(-7.5f, 0.2f, -6.5f);
    private Sphere bb = new
                      Sphere(16, 16, 0.2f);
    public static SwordSamuri swordsamuri = new SwordSamuri(6, 2, 0, 14);
    ...
    @Override
        ...
        if(GameStart.right%6==3){
         ...
         }else if(swordsamuri.execute==5&&GameStart.right%6==3){
             GameStart.right++;
             swordsamuri.energy =2;
             currentEnergy = swordsamuri.energy;
             swordsamuri.execute=4;
         }else if(swordsamuri.execute==4&&currentEnergy==0&&GameStart.right%6==3){
             GameStart.right++;
             swordsamuri.energy =4;
             currentEnergy = swordsamuri.energy;  
             swordsamuri.execute=2;
         }else if(swordsamuri.execute==2&&currentEnergy==0&&GameStart.right%6==3){
             GameStart.right++;
    ...
    @Override
         public void initialize(AppStateManager stateManager,
            ...
            this.inputManager = this.app.getInputManager();
             ...
             pawn = assetManager.loadModel("Textures/chesspawn/swordC0"
                     + ".j3o");
           ...
           System.out.println("You triggered: "+name);          
                   if(name.equals(MAPPING_Up) && !isPressed) {
                       if(clicktime==0){
                        ...
                        swordsamuri.pz--;
                        GameStart.chessmap.setmap2(swordsamuri.px, swordsamuri.pz,6);
                        }
                      }else{
                       pawn.move(0.0f, -0.5f, 0.0f);
                       chagedirectoinn(SamuriTestState4.swordsamuri.getDirection(), 1);
                   ...
                   if (name.equals(MAPPING_Down) && !isPressed) {
                       ...
                       pawn.move(0.0f, -0.5f, 0.0f);
                       chagedirectoinn(SamuriTestState4.swordsamuri.getDirection(), 0);
                    ...
                    if (name.equals(MAPPING_Left) && !isPressed) {
                       ...
                       pawn.move(0.0f, -0.5f, 0.0f);
                       chagedirectoinn(SamuriTestState4.swordsamuri.getDirection(), 3);
                   ...
                   if (name.equals(MAPPING_Right) && !isPressed) {
                       ...
                       pawn.move(0.0f, -0.5f, 0.0f);
                       chagedirectoinn(SamuriTestState4.swordsamuri.getDirection(), 2);
               ...
               rootNode.collideWith(ray, results);
                ...
                if (results.size() > 0) {
                        Geometry target = results.getClosestCollision().getGeometry();  
                         if(target.getName().equals("ball4")&&clicktime==0){
