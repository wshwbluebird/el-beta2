 ...
 * @author wshwbluebird
 */
public class SamuriTestState5 extends AbstractAppState{
    private int clicktime = 0;
    private Vector3f locbase = new Vector3f(-7.5f, 0.2f, -6.5f);
   
    public static AxSamuri axsamuri = new AxSamuri(2, 1, 7, 0);
    ...
    @Override
        ...
        if(GameStart.right%6==4){
          ...
          if(axsamuri.execute==0&&currentEnergy==0){
             ...
             currentEnergy = axsamuri.energy;  
             axsamuri.execute=5;
         }else if(axsamuri.execute==5&&GameStart.right%6==4){
             GameStart.right++;
             axsamuri.energy =2;
             currentEnergy = axsamuri.energy;
             axsamuri.execute=4;
         }else if(axsamuri.execute==4&&currentEnergy==0&&GameStart.right%6==4){
             GameStart.right++;
             axsamuri.energy =4;
             currentEnergy = axsamuri.energy;  
             axsamuri.execute=2;
         }else if(axsamuri.execute==2&&currentEnergy==0&&GameStart.right%6==4){
             GameStart.right++;
             axsamuri.energy =6;
             currentEnergy = axsamuri.energy;  
             axsamuri.execute=0;
    ...
    @Override
         public void initialize(AppStateManager stateManager,
            ...
            this.inputManager = this.app.getInputManager();
             ...
             pawn = assetManager.loadModel("Textures/chesspawn/axeC.j3o");
           ...
           System.out.println("You triggered: "+name);          
                   if(name.equals(MAPPING_Up) && !isPressed) {
                       if(clicktime==0){
                        ...
                        axsamuri.pz--;
                        GameStart.chessmap.setmap2(axsamuri.px, axsamuri.pz,2);
                        }
                      }else{
                       pawn.move(0.0f, -0.5f, 0.0f);
                       chagedirectoinn(SamuriTestState5.axsamuri.getDirection(), 1);
                   ...
                   if (name.equals(MAPPING_Down) && !isPressed) {
                       ...
                       pawn.move(0.0f, -0.5f, 0.0f);
                       chagedirectoinn(SamuriTestState5.axsamuri.getDirection(), 0);
                    ...
                    if (name.equals(MAPPING_Left) && !isPressed) {
                       ...
                       pawn.move(0.0f, -0.5f, 0.0f);
                       chagedirectoinn(SamuriTestState5.axsamuri.getDirection(), 3);
                   ...
                   if (name.equals(MAPPING_Right) && !isPressed) {
                       ...
                       pawn.move(0.0f, -0.5f, 0.0f);
                       chagedirectoinn(SamuriTestState5.axsamuri.getDirection(), 2);
               ...
               rootNode.collideWith(ray, results);
                ...
                if (results.size() > 0) {
                        Geometry target = results.getClosestCollision().getGeometry();  
                         if(target.getName().equals("ball5")&&clicktime==0){
